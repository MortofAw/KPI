...
void MainWindow::on_calculateButton_clicked()
{
    ...
    QString text = QString(
        ...
        "<b>Частота поливу:</b> %4 разів на тиждень<br><br>"
        "%5")
        .arg(p.name.isEmpty() ? "Нової рослини" : p.name)
        .arg(k, 0, 'f', 2)
        .arg(qRound(water))
        .arg(freq, 0, 'f', 1)
        .arg(p.getWarning());
...
void MainWindow::currentProfileFromForm() const
{
    PlantProfile p;
...
void MainWindow::on_saveButton_clicked()
{
    PlantProfile p = currentProfileFromForm();
    if (p.name.isEmpty()) {
        QMessageBox::warning(this, "Помилка", "Введіть назву рослини");
        return;
    }
    ProfileRepository::instance().save(p);
    refreshProfileList();
    QMessageBox::information(this, "Успіх", "Профіль збережено");
}
